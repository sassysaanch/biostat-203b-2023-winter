...
hosp_dir <- file.path("/Users/saanchishah/mimic/hosp")
...
admissions_tble <- read_csv(file = file.path(input_admissions, "admissions.csv.gz")) %>% 
  print(width = Inf)
...
chartevents_tble <- read_csv("/Users/saanchishah/mimic/icu/chartevents_filtered_itemid.csv.gz", 
...
#First do a left join followed by grouping and filtering the charttimes in such a way that charttimes fall between 
...
mimic_icu_cohort %>% 
  drop_na(thirty_day_mort, ethnicity) %>% #Drop NAs because it gave me a third column
  #because it gave me an NA bar as well otherwise
  group_by(thirty_day_mort, ethnicity) %>% #To compute a proportion, first need to 
  ...
  ggtitle("Proportion of number of individuals with mortality within 30 days by ethnic groups")
