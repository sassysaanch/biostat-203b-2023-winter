...
```{bash}
gunzip -c ~/mimic/core/admissions.csv.gz | awk -F "," '{print $6}'\
| sort | uniq -c
...
```{bash}
gunzip -c ~/mimic/core/admissions.csv.gz | awk -F "," '{print $7}'\
| sort | uniq -c
```
...
```{bash}
gunzip -c ~/mimic/core/admissions.csv.gz | awk -F "," '{print $9}'\
| sort | uniq -c
```
Answer: Insurance takes on the value of Medicaid, Medicare and Other

```{bash}
gunzip -c ~/mimic/core/admissions.csv.gz | awk -F "," '{print $12}'\
| sort | uniq -c
```
...
who am i
```
This command displayed my username.
